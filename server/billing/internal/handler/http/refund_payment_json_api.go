...
func (h *Handler) RefundPaymentJsonAPI(w http.ResponseWriter, req *http.Request) (err error) {
	...
	var request billingmodel.RefundPaymentRequest
	if err := json.Unmarshal(data, &request); err != nil {
		...
		json.NewEncoder(w).Encode(server.ServerResponse{
			...
			Error: &server.ErrorCode{
				...
				Explain: "failed to parse refund payment request",
			},
		})
	...
	err = h.controller.RefundPayment(req.Context(), request.ID, user.ID)
	if err != nil {
		...
		json.NewEncoder(w).Encode(server.ServerResponse{
			...
			Error:  &server.ErrorCode{
				...
				Explain: "failed to refund payment",
			},
		})
	...
	response, err := json.Marshal(billingmodel.RefundPaymentResponse{
