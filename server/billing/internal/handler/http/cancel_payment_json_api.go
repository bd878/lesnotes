...
func (h *Handler) CancelPaymentJsonAPI(w http.ResponseWriter, req *http.Request) (err error) {
	...
	var request billingmodel.CancelPaymentRequest
	if err := json.Unmarshal(data, &request); err != nil {
		...
		json.NewEncoder(w).Encode(server.ServerResponse{
			...
			Error: &server.ErrorCode{
				...
				Explain: "failed to parse cancel invoice request",
			},
		})
	...
	err = h.controller.CancelPayment(req.Context(), request.ID, user.ID)
	if err != nil {
		...
		json.NewEncoder(w).Encode(server.ServerResponse{
			...
			Error:  &server.ErrorCode{
				...
				Explain: "failed to cancel invoice",
			},
		})
	...
	response, err := json.Marshal(billingmodel.CancelPaymentResponse{
