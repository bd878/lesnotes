...
func (r *UsersRepository) FindByID(ctx context.Context, id int64) (user *users.User, err error) {
	query := fmt.Sprintf(`
SELECT DISTINCT
	u.login,
	u.salt,
...
func (r *UsersRepository) FindByLogin(ctx context.Context, login string) (user *users.User, err error) {
	query := fmt.Sprintf(`
SELECT DISTINCT
	u.id,
	u.salt,
