...
func (r *UsersDumper) Open(ctx context.Context) (ch chan *users.UsersSnapshot, err error) {
	r.ctx, r.cancel = context.WithCancelCause(ctx)
	ch = make(chan *users.UsersSnapshot, 100)
	r.ch = ch
...
func (r *UsersDumper) runUsers() {
	...
	for rows.Next() {
		...
		r.ch <- &users.UsersSnapshot{
			Item: &users.UsersSnapshot_User{
				User: user,
			},
		}
	}
...
func (r *UsersDumper) runPremiums() {
	...
	for rows.Next() {
		premium := &users.PremiumSnapshotItem{}
		...
		r.ch <- &users.UsersSnapshot{
			Item: &users.UsersSnapshot_Premium{
				Premium: premium,
			},
		}
	}
...
func (r *UsersDumper) Restore(ctx context.Context, snapshot *users.UsersSnapshot) (err error) {
