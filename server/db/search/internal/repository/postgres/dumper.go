...
func (r *Dumper) Open(ctx context.Context) (ch chan *search.SearchSnapshot, err error) {
	r.ctx, r.cancel = context.WithCancelCause(ctx)
	ch = make(chan *search.SearchSnapshot, 100)
	r.ch = ch
...
func (r *Dumper) runMessages() {
	...
	for rows.Next() {
		message := &search.SearchMessageSnapshotItem{}
		...
		r.ch <- &search.SearchSnapshot{
			Item: &search.SearchSnapshot_Message{
				Message: message,
			},
		}
	}
...
func (r *Dumper) runFiles() {
	...
	for rows.Next() {
		file := &search.SearchFileSnapshotItem{}
		...
		r.ch <- &search.SearchSnapshot{
			Item: &search.SearchSnapshot_File{
				File: file,
			},
		}
	}
...
func (r *Dumper) runTranslations() {
	...
	for rows.Next() {
		translation := &search.SearchTranslationSnapshotItem{}
		...
		r.ch <- &search.SearchSnapshot{
			Item: &search.SearchSnapshot_Translation{
...
func (r *Dumper) runThreads() {
	...
	for rows.Next() {
		thread := &search.SearchThreadSnapshotItem{}
		...
		r.ch <- &search.SearchSnapshot{
			Item: &search.SearchSnapshot_Thread{
				Thread: thread,
			},
		}
	}
...
func (r *Dumper) Restore(ctx context.Context, snapshot *search.SearchSnapshot) (err error) {
