...
func (r *Dumper) Open(ctx context.Context) (ch chan *threads.ThreadsSnapshot, err error) {
	r.ctx, r.cancel = context.WithCancelCause(ctx)
	ch = make(chan *threads.ThreadsSnapshot, 100)
	r.ch = ch
...
func (r *Dumper) runThreads() {
	...
	for rows.Next() {
		thread := &threads.ThreadSnapshotItem{}
		...
		r.ch <- &threads.ThreadsSnapshot{
			Item: &threads.ThreadsSnapshot_Thread{
				Thread: thread,
			},
		}
	}
...
func (r *Dumper) Restore(ctx context.Context, snapshot *threads.ThreadsSnapshot) (err error) {
