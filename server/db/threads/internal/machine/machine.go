...
type Dumper interface {
	Open(ctx context.Context) (ch chan *threads.ThreadsSnapshot, err error)
	Restore(ctx context.Context, thread *threads.ThreadsSnapshot) (err error)
	Close() (err error)
}
...
func (f *Machine) applyReorder(raw []byte) interface{} {
	var cmd threads.ReorderCommand
	proto.Unmarshal(raw, &cmd)
...
func (f *Machine) applyPublish(raw []byte) interface{} {
	var cmd threads.PublishCommand
	proto.Unmarshal(raw, &cmd)
...
func (f *Machine) applyPrivate(raw []byte) interface{} {
	var cmd threads.PrivateCommand
	proto.Unmarshal(raw, &cmd)
...
func (f *Machine) applyPublishMessages(raw []byte) interface{} {
	var cmd threads.PublishMessagesCommand
	proto.Unmarshal(raw, &cmd)
...
func (f *Machine) applyPrivateMessages(raw []byte) interface{} {
	var cmd threads.PrivateMessagesCommand
	proto.Unmarshal(raw, &cmd)
