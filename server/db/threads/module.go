...
import (
	...
	"github.com/bd878/gallery/server/db/threads/config"
	"github.com/bd878/gallery/server/db/threads/internal/repository/postgres"
	"github.com/bd878/gallery/server/db/threads/internal/machine"
	"github.com/bd878/gallery/server/db/threads/internal/controller/distributed"
	"github.com/bd878/gallery/server/db/threads/internal/handler/grpc"
)
...
func Root(ctx context.Context, cfg config.Config, svc system.Service) (err error) {
	...
	controller := application.New(consensus, threadsRepo)

	handler := grpc.New(controller)

	threads.RegisterThreadsServer(svc.RPC(), handler)
...
func setupRaft(svc system.Service, cfg config.Config, threadsRepo *postgres.ThreadsRepository, dumper *postgres.Dumper) (*raft.Distributed, error) {
	fsm := machine.New(threadsRepo, dumper)
