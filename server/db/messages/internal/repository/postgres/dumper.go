package postgres

import (
	"context"
	"errors"
	"fmt"
	"log/slog"
	"sync"
	"time"

	"github.com/jackc/pgx/v5/pgxpool"

	"github.com/bd878/gallery/server/api/comments"
	"github.com/bd878/gallery/server/api/messages"
	"github.com/bd878/gallery/server/api/translations"
)

type Dumper struct {
	pool                  *pgxpool.Pool
	messagesTableName     string
	translationsTableName string
	commentsTableName     string
	ctx                   context.Context
	cancel                context.CancelCauseFunc
	ch                    chan *messages.MessagesSnapshot
	wg                    sync.WaitGroup
}

func NewDumper(pool *pgxpool.Pool, messagesTableName,
	translationsTableName, commentsTableName string) *Dumper {
	return &Dumper{
		pool:                  pool,
		messagesTableName:     messagesTableName,
		translationsTableName: translationsTableName,
		commentsTableName:     commentsTableName,
	}
}

func (r *Dumper) Open(ctx context.Context) (ch chan *messages.MessagesSnapshot, err error) {
	r.ctx, r.cancel = context.WithCancelCause(ctx)
	ch = make(chan *messages.MessagesSnapshot, 100)
	r.ch = ch

	r.wg.Add(1)
	go r.runMessages()
	r.wg.Add(1)
	go r.runTranslations()
	r.wg.Add(1)
	go r.runComments()

	go func() {
		r.wg.Wait()
		close(r.ch)
	}()

	return
}

func (r *Dumper) runMessages() {
	query := "SELECT id, text, private, name, user_id, title, created_at, updated_at FROM %s"

	defer r.wg.Done()
	defer slog.Debug("messages dump finished")

	rows, err := r.pool.Query(r.ctx, r.messagesTable(query))
	if err != nil {
		slog.Error(err.Error())
		r.cancel(err)
		return
	}

	defer rows.Close()

	for rows.Next() {
		message := &messages.MessageSnapshotItem{}

		var createdAt, updatedAt *time.Time
		err = rows.Scan(&message.Id, &message.Text, &message.Private, &message.Name,
			&message.UserId, &message.Title, &createdAt, &updatedAt)
		if err != nil {
			slog.Error(err.Error())
			r.cancel(err)
			return
		}

		message.CreatedAt = createdAt.Format(time.RFC3339)
		message.UpdatedAt = updatedAt.Format(time.RFC3339)

		select {
		case <-r.ctx.Done():
			return
		default:
		}

		r.ch <- &messages.MessagesSnapshot{
			Item: &messages.MessagesSnapshot_Message{
				Message: message,
			},
		}
	}

	if err := rows.Err(); err != nil {
		slog.Error(err.Error())
		r.cancel(err)
		return
	}
}

func (r *Dumper) runTranslations() {
	query := "SELECT message_id, lang, text, title, created_at, updated_at FROM %s"

	defer r.wg.Done()
	defer slog.Debug("translations dump finished")

	rows, err := r.pool.Query(r.ctx, r.translationsTable(query))
	if err != nil {
		slog.Error(err.Error())
		r.cancel(err)
		return
	}

	defer rows.Close()

	for rows.Next() {
		translation := &translations.TranslationSnapshotItem{}

		var createdAt, updatedAt *time.Time
		err = rows.Scan(&translation.Id, &translation.Lang, &translation.Text,
			&translation.Title, &createdAt, &updatedAt)
		if err != nil {
			slog.Error(err.Error())
			r.cancel(err)
			return
		}

		translation.CreatedAt = createdAt.Format(time.RFC3339)
		translation.UpdatedAt = updatedAt.Format(time.RFC3339)

		select {
		case <-r.ctx.Done():
			return
		default:
		}

		r.ch <- &messages.MessagesSnapshot{
			Item: &messages.MessagesSnapshot_Translation{
				Translation: translation,
			},
		}
	}

	if err := rows.Err(); err != nil {
		slog.Error(err.Error())
		r.cancel(err)
		return
	}
}

func (r *Dumper) runComments() {
	query := "SELECT id, message_id, user_id, text, metadata, created_at, updated_at FROM %s"

	defer r.wg.Done()
	defer slog.Debug("comments dump finished")

	rows, err := r.pool.Query(r.ctx, r.commentsTable(query))
	if err != nil {
		slog.Error(err.Error())
		r.cancel(err)
		return
	}

	defer rows.Close()

	for rows.Next() {
		comment := &comments.CommentSnapshotItem{}

		var createdAt, updatedAt time.Time
		err = rows.Scan(&comment.Id, &comment.MessageId, &comment.UserId, &comment.Text,
			&comment.Metadata, &createdAt, &updatedAt)
		if err != nil {
			slog.Error(err.Error())
			r.cancel(err)
			return
		}

		comment.CreatedAt = createdAt.Format(time.RFC3339)
		comment.UpdatedAt = updatedAt.Format(time.RFC3339)

		select {
		case <-r.ctx.Done():
			return
		default:
		}

		r.ch <- &messages.MessagesSnapshot{
			Item: &messages.MessagesSnapshot_Comment{
				Comment: comment,
			},
		}
	}

	if err := rows.Err(); err != nil {
		slog.Error(err.Error())
		r.cancel(err)
		return
	}
}

func (r *Dumper) Close() (err error) {
	slog.Debug("close dumper")
	r.cancel(nil)
	r.wg.Wait()
	return nil
}

func (r *Dumper) Restore(ctx context.Context, snapshot *messages.MessagesSnapshot) (err error) {
	switch v := snapshot.Item.(type) {
	case *messages.MessagesSnapshot_Message:

		query := "INSERT INTO %s(id, text, private, name, user_id, title, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)"

		_, err = r.pool.Exec(ctx, r.messagesTable(query), v.Message.Id, v.Message.Text, v.Message.Private,
			v.Message.Name, v.Message.UserId, v.Message.Title, v.Message.CreatedAt, v.Message.UpdatedAt)

		return

	case *messages.MessagesSnapshot_Translation:

		query := "INSERT INTO %s(message_id, lang, text, title, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$6)"

		_, err = r.pool.Exec(ctx, r.translationsTable(query), v.Translation.Id, v.Translation.Lang, v.Translation.Text,
			v.Translation.Title, v.Translation.CreatedAt, v.Translation.UpdatedAt)

		return

	case *messages.MessagesSnapshot_Comment:

		query := "INSERT INTO %s(id, message_id, user_id, text, metadata, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7)"

		_, err = r.pool.Exec(ctx, r.commentsTable(query), v.Comment.Id, v.Comment.MessageId, v.Comment.UserId, v.Comment.Text,
			v.Comment.Metadata, v.Comment.CreatedAt, v.Comment.UpdatedAt)

		return

	default:
		return errors.New("unknown snapshot item")
	}
}

func (r Dumper) messagesTable(query string) string {
	return fmt.Sprintf(query, r.messagesTableName)
}

func (r Dumper) translationsTable(query string) string {
	return fmt.Sprintf(query, r.translationsTableName)
}

func (r Dumper) commentsTable(query string) string {
	return fmt.Sprintf(query, r.commentsTableName)
}
