...
func (p *Picker) Pick(info balancer.PickInfo) (
	...
	if strings.Contains(info.FullMethodName, "SaveMessage") ||
		 ...
		 strings.Contains(info.FullMethodName, "PublishMessage") ||
		 strings.Contains(info.FullMethodName, "PrivateMessage") ||
		 strings.Contains(info.FullMethodName, "UpdateMessage") ||
		len(p.followers) == 0 {
			result.SubConn = p.leader
	} else if strings.Contains(info.FullMethodName, "SearchMessages") {
