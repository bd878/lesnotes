...
type Repository interface {
	...
	Update(ctx context.Context, userID, id int64, newText, newTitle, newName string, newFileIDs []int64, newPrivate int) (err error)
	...
	Count(ctx context.Context, userID int64) (count int, err error)
...
func (f *fsm) applyUpdate(raw []byte) interface{} {
	...
	err := f.repo.Update(context.Background(), cmd.UserId, cmd.Id, cmd.Text, cmd.Title, cmd.Name, cmd.FileIds, int(cmd.Private))
	if err != nil {
		return err
	}
