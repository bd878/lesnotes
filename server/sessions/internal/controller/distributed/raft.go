...
type DistributedSessions struct {
	conf            Config
	raft            *raft.Raft
	repo            Repository
...
func New(conf Config, repo Repository) (*DistributedSessions, error) {
...
func (m *DistributedSessions) setupRaft() error {
	fsm := &fsm{repo: m.repo}
...
func (m *DistributedSessions) WaitForLeader(timeout time.Duration) error {
...
func (m *DistributedSessions) GetServers(_ context.Context) ([](*api.Server), error) {
...
func (m *DistributedSessions) Join(id, addr string) error {
...
func (m *DistributedSessions) Leave(id string) error {
	leaderFuture := m.raft.VerifyLeader()
	if err := leaderFuture.Error(); err != nil {
...
func (m *DistributedSessions) Snapshot() error {
...
func (m *DistributedSessions) Restore() error {
	leaderFuture := m.raft.VerifyLeader()
	if err := leaderFuture.Error(); err != nil {
...
func (m *DistributedSessions) ShowLeader() error {
	state := m.raft.State()
